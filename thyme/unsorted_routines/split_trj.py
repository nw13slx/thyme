...
logging.basicConfig(
    filename=f"{folder}/split_{name}.log",
    filemode="w",
    level=logging.DEBUG,
    format="%(message)s",
)
...
def o_dist_filter(o_xyz, f, e, c, species):
    ...
    atoms = Structure(lattice=c, species=species, coords=xyz, coords_are_cartesian=True)
    ...
    for i, trjname in enumerate(trjnames):
        ...
        if frame > 0:
            ...
            if md == 0:
                ...
                this_valid = np.min(
                    (
                        int(np.ceil(nvalid / total_frames * skip_frame)),
                        skip_frame - this_train,
                    )
                )
                this_test = np.min(
                    (
                        int(np.ceil(ntest / total_frames * skip_frame)),
                        skip_frame - this_valid - this_train,
                    )
                )
                this_spare = np.max(
                ...
                print(
                    trjname, skip_frame, this_train, this_valid, this_test, this_spare
                )
                ...
                if this_test > 0:
                    list_to["test"] = id_list[
                        this_train + this_valid : this_test + this_train + this_valid
                    ]
                if this_spare > 0:
